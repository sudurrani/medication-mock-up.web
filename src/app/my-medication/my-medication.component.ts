...
export class MyMedicationComponent implements OnInit {
  ...
  displayedColumns = ['id','pin', 'medicationName', 'dosage', 'prescriptionDate', 'displayName', 'createdDate', 'actions'];
  ...
  async getMy() {
    ...
    var inputJSON = `{"id":"${user.id}"}`;
    await this._httpService.get('Medication/GetMy/' + user.id)
